...
function Validation() {
    ...
    if ($('.moneyasked input').val().length == 0 || $('.moneyasked input').val() == 'Ύψος χρημάτων που ζητήθηκαν') {
        IsValid = false
        $('.moneyasked').removeClass('moneytxt').addClass('moneytxt-error')
    } else {
        $('.moneyasked').removeClass('moneytxt-error').addClass('moneytxt')
    }

    if ($('.moneygiven input').val().length == 0 || $('.moneygiven input').val() == 'Χρήματα που τελικά πλήρωσα') {
        IsValid = false
        $('.moneygiven').removeClass('moneytxt').addClass('moneytxt-error')
    } else {
        $('.moneygiven').removeClass('moneytxt-error').addClass('moneytxt')
    }
    ...
    if ($('.comment textarea').val().length == 0 || $('.comment textarea').val() == 'Γράψε εδώ μία σύντομη περιγραφή του περιστατικού') {
        IsValid = false
        $('.comment').removeClass('commentstxt').addClass('commentstxt-error')
    } else {
        $('.comment').removeClass('commentstxt-error').addClass('commentstxt')
    }
